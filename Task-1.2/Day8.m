...
subplot(2,2,1);
impulse(closed_tf, t);
title("Impulse Response with PID: Kp = 100,Ki =10, Kd = 30");
grid on;

subplot(2,2,2);
step(closed_tf, t);
title("Step Response with PID: Kp = 100,Kp = 10, Kd = 30");
grid on;

subplot(2,2,3);
pzmap(closed_tf);
title("Pole-zero map of inverted pendulum system with PID Controller");
grid on;